...
$file = $ROOTDIR . '/a/' . $current_catalogue['login'] . '/catalog.xlsx';
$group_field_white = [
...
foreach ($groups as $group_id => $group) {
    ...
    foreach ($group_field_white_key as $col => $field) {
        $worksheet->setCellValueByColumnAndRow(($col + 1), $row, $group[$field]);
    }
    echo "c- ";
    flush();
    $row++;
    foreach ($groupItems as $item) {
        foreach ($item_field_white_key as $col => $field) {
            $worksheet->setCellValueByColumnAndRow(($col + 2), $row, $item[$field]);
        }
        if (!empty($item['params'])) {
            ...
            $col = count($item_field_white) + 2;
            foreach ($params as $param) {
                if (isset($params_list[$param['key']])) {
                    $worksheet->setCellValueByColumnAndRow($col++, $row, $params_list[$param['key']]);
                    $worksheet->setCellValueByColumnAndRow($col++, $row, $param['value']);
                }
            }
        }
        $row++;
...
for ($i=0; $i <= $total_cols_param; $i++) { 
    $item_field_white["paramName{$i}"] = 'Наименование параметра';
    $item_field_white["paramValue{$i}"] = 'Значение параметра';
}
$item_field_white_key = array_keys($item_field_white);
...
foreach ($header as $col => $key) {
    $worksheet->setCellValueByColumnAndRow($col + 1, 1, $key);
}
