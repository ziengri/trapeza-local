...
// Занесем его в сессию
    $_SESSION["PreviewClass"][$classPreview] = array(
            ...
            "AddActionTemplate" => $_POST["AddActionTemplate"]);
}
...
if (!isset($sub) || !$sub) {
    ...
    switch (true) {
        ...
        case ($sub_count == 1):
            $Location = $SUB_FOLDER.$HTTP_ROOT_PATH."add.php?catalogue=".$catalogue."&sub=".$res_arr[0]['Subdivision_ID']."&cc=".$res_arr[0]['Sub_Class_ID']."&classPreview=".$classPreview;
        ...
        case ($sub_count > 1):
            ...
            foreach ($res_arr as $tmp_arr) {
                ...
                echo "<li><a href='".$SUB_FOLDER.$HTTP_ROOT_PATH."add.php?catalogue=".$catalogue."&amp;sub=".$sub."&amp;cc=".$cc."&amp;classPreview=".$classPreview."'>".$cc." . ".$cc_name."</a></li>";
            }
            echo "</ul></div>";
            EndHtml();
            die();
