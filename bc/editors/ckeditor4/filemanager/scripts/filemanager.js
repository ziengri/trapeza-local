...
var cleanString = function(str) {
	...
	// Если кирилица запрещена, то транслитерируем её
	if(!CKEditorAllowCyrilicFolder) {
		//cleaned = cleaned.replace(/[^_a-zA-Z0-9]/g, "");

        var p_search = new Array(                       // Cyrilic
                "А", "а", "Б", "б", "В", "в", "Г", "г",
            "Д", "д", "Е", "е", "Ё", "ё", "Ж", "ж",
            "З", "з", "И", "и", "Й", "й", "КС", "кс",
            "К", "к", "Л", "л", "М", "м", "Н", "н",
...
function has_capability(data, cap) {
	if (data['File Type'] == 'dir' && (cap == 'download' || cap == 'crop')) return false;
...
var bindToolbar = function(data){
    ...
    if (!has_capability(data, 'crop')) {
        $('#fileinfo').find('button#crop').hide();
    } else {
        $('#fileinfo').find('button#crop').click(function(){
            if(cropItem(data)) $('#fileinfo').html('');
        }).show();
    }
...
var cropItem = function (data) {
    //alert('crop');


    var img_src = $('#preview img').attr('src');

    var img_src_w = $('#preview img').attr('width');
    var img_src_w = $('#preview img').attr('height');
    ...
    $('#preview h1').before("<button id='do_crop' name='do_crop' type='button' style='z-index: 500; margin-top: 15px; float: left;'><span>" + lg.crop + "</span></button>");
    $('#do_crop').attr('disabled', true);
       ...
       $('#do_crop').click(function(){
           ...
           $.ajax({
               ...
               success: function(result){
                   ...
                   $('#preview img').removeAttr("src").css("height",'').css("width",'').css("max-width",img_src_w).attr("src", img_src);
...
var updateCoords = function (c) {
        ...
        $('#img').val($('#fileinfo img').attr('src'));

        $('#do_crop').attr('disabled', false);
...
var getFileInfo = function(file){
    ...
    template += '<button id="crop" name="crop" type="button" value="Crop">' + lg.crop + '</button>';
