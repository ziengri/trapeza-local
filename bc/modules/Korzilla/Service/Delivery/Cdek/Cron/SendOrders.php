...
use App\modules\Korzilla\Service\Delivery\Cdek\DbOrderParser;
use Custom\Modules\Korzilla\Service\Delivery\Cdek\DbOrderParser as CutomDbOrderParser;
...
class SendOrders
{
    ...
    public function __construct(int $catalogueId)
    {
        ...
        $this->locker = new Loker('cdekSendOrders');
        $this->cdek = Cdek::getInstance();
    }

    public function handle()
    {   
        if (!$this->cdek->isOn() || $this->locker->isLocked(self::LOCK_TIME_LIMIT)) {
            return;
        }
        ...
        foreach ($this->getOrders() as $order) {
            $this->locker->lock();
            try {
                if (class_exists('Custom\Modules\Korzilla\Service\Delivery\Cdek\DbOrderParser')) {
                    $orderParser = new CutomDbOrderParser($order);
                } else {
                    $orderParser = new DbOrderParser($order);
                }                
                $response = $this->cdek->orderRegistrate($orderParser);
            } catch (Exception $e) {
    ...
    {
        $sql = "UPDATE `Message2005` SET `code` = '{$response['entity']['uuid']}' WHERE `Message_ID` = {$order['Message_ID']}";
    ...
    private function clearOldLogs()
    {
        ...
        $logDir = $_SERVER['DOCUMENT_ROOT'].$pathInc.'/log/Cdek/orders';
