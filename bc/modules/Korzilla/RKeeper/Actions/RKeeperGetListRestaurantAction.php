...
class RKeeperGetListRestaurantAction
{
     ...
     */
    public function run(string $clientId) : array
    {
        $token = $this->getTokenSubAction->run();

        $getMenuRequest = new RKeeperRequestDTO();

        $getMenuRequest->url = RKeeperConfig::BASE_URL . "/orderSources/" . $clientId . "/restaurants";
