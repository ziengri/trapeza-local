...
if($('body').hasClass('noauth')){
	lightcase.start({
		href: "/profile/?isNaked=1&sites=1",
		maxWidth: 320,
		groupClass: "login noclose",
		title: "Авторизация",
...
$(document).ready(function(){
	...
	// SEARCH
	$('body').on('keyup', '.ll-input input', function(event){
		inputText = event.target.value.toLowerCase();
		if(inputText == ""){
			$('.ll-site, #middle-line .rowline').show();
		}else {
			$('.ll-site').hide().each(function(){
				item = $(this);
				name = item.find('.ll-name').text().toLowerCase();
				href = item.find('.ll-name').parent().attr("href");
				if(name.indexOf(inputText) + 1 || href.indexOf(inputText) + 1){
					item.show();
				}
			});
			$('#middle-line .rowline').hide().each(function(){
				item = $(this);
				name = item.find('.bb-infoline .il-body a').text().toLowerCase();
				name2 = item.find('.bh-name a').text().toLowerCase();

				if(name.indexOf(inputText) + 1 || name2.indexOf(inputText) + 1){
					item.show();
				}
			});
		}
    ...
    Materialize.updateTextFields = function() {
        var input_selector = 'input[type=text], input[type=password], input[type=email], input[type=url], input[type=tel], input[type=number], input[type=search]';
...
$(document).on("change", ".bc_inpt .switch input", function(){
	$('#middle-line .rowline').show();
	$(this).parents(".bc_inpt").find('.switch input:checked').each(function(){
		name = $(this).attr('name');
		if(name=='checkDomain'){
			$("#middle-line .rowline a[href$='krzi.ru']").parents(".rowline").hide()
		}
		if(name=='checkConcept'){
			$("#middle-line .rowline:not([data-concept])").hide()
		}
	});
});
...
function loadingView(param, where) {
	...
	paramEnd = {'transition': '250ms ease'};
