...
(function($) {
	var methods = {
		...
		addMainElements: function() {
			...
			methods.builder.append(methods.builder.btnsmain = $('<div class="fb-btns-main"><div class="fb-btns-left"></div><div class="fb-btns-right"></div></div>'),
			methods.builder.main = $('<div class="fb-main"></div>'));
			methods.builder.btnsmain.find('.fb-btns-left').append(methods.getZone);
			methods.builder.btnsmain.find(".fb-btns-right").append(methods.builder.btns = $('<div class="fb-btns"></div>'))
			methods.builder.main.append(methods.builder.mainFields = $('<div class="fb-main-fields cb"></div>'));
		...
		jqueryui: function() {
			...
			var sortJson = {
				connectWith: '.fb-main-fields .fields-header',
				items: "> .fb-btn",
				cursor: "move"
			}
			getWidth = function(event){
				...
				helper = $('.ui-resizable-helper').width();
				wPrec = (100/w)*helper;
				wP = Math.ceil(wPrec / 5) * 5;
				if(wP>100) wP = 100
				element.css('width', (wP-1)+'%').data('wp', wP)
			}
			methods.builder.find(".fb-main-fields").sortable({
				revert: true
			});
    		$( ".fb-main-fields" ).disableSelection();
			methods.builder.find(".fb-btns-left .fb-btn-zone").draggable({
				...
				stop: function(event, ui) {
					...
					if(element.parents('.fb-btn-zone').length) element = element.parents('.fb-btn-zone')
					element.css('width', '99%').data('wp', 100)
					element.resizable(resizeJson)
					element.find('.fields-header').sortable(sortJson);
					lightcaseStyle();
				}
			})
			methods.builder.find(".fb-btns-right .fb-btn").draggable({
				connectToSortable: ".fb-main-fields .fields-header",
				helper: "clone",
				revert: "invalid",
				revertDuration: 0,
				stop: function(event, ui) {
					lightcaseStyle();
					element = $(event.toElement)
					if(element.parents('.fb-btn').length) element = element.parents('.fb-btn')
					element.css('width', '99%').data('wp', 100)
					element.resizable(resizeJson);
				}
			});
			methods.builder.find(".fb-main-fields .fields-header").sortable(sortJson);
			methods.builder.find(".fb-main-fields .fb-btn-zone").resizable(resizeJson);
			methods.builder.find(".fb-btns-left .fb-btn-zone").disableSelection();
			...
			methods.builder.find(".fb-btn-zone").draggable({
				connectToSortable: "#formBuilder .fb-main-fields",
				helper: "clone",
				revert: "invalid",
			...
			methods.builder.find(".fb-btn").draggable({
				connectToSortable: "#formBuilder .fb-btn-zone",
				helper: "clone",
				revert: "invalid",
				revertDuration: 0,
				create: function( event, ui ) {
		...
		getZone: function(j, zone_id) {
			data = ""
			style = ""
			if(typeof j === 'object'){
				data += ' data-wp="'+j['size']+'"';
				data += ' data-id="'+zone_id+'"';
				style += 'style="width: '+(j['size']-1)+'%;"';
			}
			result = "<div class='fb-btn-zone cb' "+style+" "+data+"><div class='fields-header'></div></div>";

			if(typeof j === 'object') methods.builder.mainFields.append(result);
			else return result;
		},
		getInput: function(j, zone_id) {
			param = {
			...
			if(typeof j === 'object') methods.builder.mainFields.find('.fb-btn-zone[data-id="'+zone_id+'"] .fields-header').append(result);
			else return result;
		},
		getTextarea: function(j, zone_id) {
			param = {
				required: { },
				label: { grid: 2 },
				name: { grid: 2 },
		...
		getTitle: function(j) {
			param = {
				label: { value: 'Строка Название', grid: 1 }
			}
			if(typeof j === 'object') $.extend(true, param, j);
			result = methods.getElementName('Название', param);
			result += methods.getElementParam(param);
			result += methods.getElementEdit();
		...
		getElementName: function(name, param) {
			return "<span class='fb-btext'>"+name+"</span><span class='fb-btext none'><span class='fb-btext-name'>"+(param.label.value ? param.label.value : "")+"</span><span class='red "+(typeof param.required !== 'undefined' && typeof param.required.value !== 'undefined' && param.required.value ? "" : "none")+"'>*</span></span>";
		},
		...
		getElementParam: function(param) {
			...
			for (var i = 0 in param) {
				switch (i) {
					...
					break;
					case 'label':
						result += methods.templateInput("label", "Название", param[i]["value"], param[i]["grid"], 1);
					break;
					case 'name':
		...
		templateInput: function(name, label, value, grid = 1, req) {
			return "<div class='fb-line fb-line-"+grid+"' data-name='"+name+"'><div class='input-field'><input type='text' name='"+(name ? name : "input")+"' "+(value ? "value='"+value+"'" : "")+" "+(label=="Название" ? "onchange='formbuilder_strtourl(this);' onkeyup='formbuilder_nametohead(this);'" : "")+" "+(req ? "data-req='"+label+"'" : "")+"><label for='input' "+(value ? "class='active'" : "")+">"+(label ? label : "")+(req ? "<span class='red'>*</span>" : "")+"</label><span></span></div></div>";
		},
		...
		setJson: function(obj) {
			console.log(obj)
			for (var zone in obj) {
				var zone_id = zone.split('|')[1];
				methods.getZone(obj[zone], zone_id);

				for (var name in obj[zone]['btns']) {
					nameElement = name.split('|')[0];
					switch (nameElement) {
						case 'input':
							methods.getInput(obj[zone]['btns'][name], zone_id);
						break;
						case 'textarea':
							methods.getTextarea(obj[zone]['btns'][name], zone_id);
						break;
						/*case 'select':
		...
		getJson: function() {
			...
			if(noReq){
				...
				builder.find(".fb-main .fb-btn-zone").each(function(i, e){
					var zone = $(this),
						btns = {};

					zone.find("[data-element]").each(function(i, e){
						var el = $(e),
						...
						el.find(".fb-param > .fb-line[data-name]").each(function(i, e){
							...
							btns[name][s]['value'] = value;
						});
						el.find("[data-options]").each(function(i, e){
							s = $(e).attr("data-options");
							btns[s] = {values: {}};
							$(e).find("[data-name='option']").each(function(i, e){
								btns[name][s]['values'][i] = {};
								btns[name][s]['values'][i]["value"] = $(this).find('input').val()
							});
						});
					});
					name = "zone|"+Math.floor(Math.random()*1000000);
					json[name] = {
						'size': zone.data('wp'),
						'btns': btns
					}
	...
	$.fn.formBuilderComponent = function(method) {
		if (methods[method]) {
