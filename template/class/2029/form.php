...
<div class="modal-body tabs-body">
	<div id='tab_t1'>
		...
		<div class='colline colline-height'>
			<?=bc_radio("f_updateprice", "0", "Удалить все товары на сайте (удаляются фотографии)", ($f_updateprice==0 ? "checked" : ""))?>
			<?=bc_radio("f_updateprice", "1", "Обновление: категории и товары", ($f_updateprice==1 || !$f_updateprice ? "checked" : ""))?>
			<?=bc_radio("f_updateprice", "2", "Обновление: только товары (без изменения категорий)", ($f_updateprice==2 ? "checked" : ""))?>
			<?=bc_radio("f_updateprice", "3", "Обновление: только цены (без изменения категорий и других колонок товара)", ($f_updateprice==3 ? "checked" : ""))?>
			<?=bc_radio("f_updateprice", "4", "Обновление: не выключая уже существующие", ($f_updateprice==4 ? "checked" : ""))?>
			<?=bc_radio("f_updateprice", "5", "Обновление: только цены (сравнивать товары только по артикулу)", ($f_updateprice==5 ? "checked" : ""))?>
		</div>
	</div>
	<div class='none' id='tab_t2'>
		...
		<div class='colline colline-5-2'><?=bc_input("f_gr", ($f_gr ? $f_gr : 1), "№ столбца с разделами", "maxlength='12' size='12'", 1)?></div>
		<div class='colline colline-5-3'><?=bc_select("f_parent", $catsubOpt, "Выгрузить в раздел", "class='ns'")?></div>
		<div class='colline colline-5-2' <?=$f_ncctpl?>><?=bc_select("f_ncctpl", $optionsNcctpl, "Шаблон отображения", "class='ns'")?></div>
		<div class='colline colline-height'><?=bc_textarea("f_pole", $f_pole, "Поля через запятую (последовательность как в файле)", "", 1)?></div>
	</div>
	<div class='none' id='tab_t3'>
		<div class='colline colline-2'><?=bc_select("f_currency", getOptionsFromArray($currencyArr, $f_currency), "Валюта в прайсе", "class='ns'")?></div>
