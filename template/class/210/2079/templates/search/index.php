<?php  require $settings_array[template_path].'top.php';?>
    ...
    <style> span.modal-vin-info-close { position: absolute; right: 15px; top: 15px; z-index: 10; cursor: pointer; height: 16px; width: 16px; background-image: url(https://212709.selcdn.ru/autocatalog-online/public/images/info-close.png); background-size: 16px 16px; }</style>
    ...
    if (count($vins) > 0 || count($frames) > 0) {
        ...
        foreach ($itemsList as $index => $vin) {
            switch (property_exists($vin, 'mark')) {
                ...
                case (in_array($vin->mark, array('FIAT', 'LANCIA', 'ALFA_ROMEO', 'ABARTH'))):
                    $url = "{$hrefPrefix}?type={$vin->type}&mark={$vin->mark}}&model={$vin->model_short_name}&modification={$vin->modification_short_name}";
                    $name = "{$vin->mark} {$vin->modification_name}";
                    ?><a href="<?php echo $url ?>"><div><span><?php echo $index+1 ?>. </span><?php echo $name ?></div></a><?php
                    break;
                case (in_array($vin->mark, array('BMW', 'ROLLS-ROYCE', 'MINI'))):
                    $url = "{$hrefPrefix}?type={$vin->type}&mark={$vin->mark}&series={$vin->series_short_name}&model={$vin->model_short_name}&rule={$vin->rule_position}&transmission={$vin->transmission}";
                ...
                case (in_array($vin->mark, array('TOYOTA', 'LEXUS'))):
                    ...
                    ?><a href="<?php echo $url ?>"><div><span><?php echo $index+1 ?>. </span><?php echo $name ?></div></a><?php
                    break;
                case (in_array($vin->mark, array('AUDI', 'SEAT', 'SKODA', 'VOLKSWAGEN'))):
                    $i = 1;
        ...
        <?php
            $getnames = array(0 => 'type', 1 => 'mark', 2 => 'modelId');
            # series model modelId
            foreach ($marks as $ind => $mark) {
                $url = $hrefPrefix."?";
                $name = ""; ?>
                <?php foreach ($mark->breadcrumbs as $key => $item) {
                    $url .= "{$getnames[$key]}={$item->url}";
                    if ($key < (count($mark->breadcrumbs) - 1))  $url .= '&';
                    $name .= $item->name . ' ';
                }
                ?>
                <a href="<?php echo $url ?>">
