...
<?php 
    global $pathInc2, $current_catalogue;
    $thisFile = $DOCUMENT_ROOT.$pathInc2."/template/class/210/2079/templates/a2d/numbers.php";
...
?>
            ...
            $(".imageLayout .ladel")
                ...
                $(".imageLayout .ladel")
                    .dblclick(function () {
                        ...
                        $("html, body")
                            .animate({
                                scrollTop: $(".table_main")
                                    .find("[data-index='" + a + "']").addClass('active')
                                    .first()
                                    .offset()
                                    .top - 70
                            }, 1e3)
                ...
                $(".table-item").click(function(){
                    ...
                    $("#imageLayout .ladel[data-index='"+index+"']").click()
                })
    ...
    <?php
    $nextUrl = $next ? $hrefPrefix."?type={$prev->type}&mark={$prev->mark}&modelId={$prev->model}&groupId={$prev->short_name}" : null;
    $nextUrl = $next ? $hrefPrefix."?type={$next->type}&mark={$next->mark}&modelId={$next->model}&groupId={$next->short_name}" : null;
    ...
    if (property_exists($group,'coordinates')) {
        foreach ($group->coordinates as $item) {
            $url = "{$hrefPrefix}?type={$breadcrumbs[1]->url}&mark={$breadcrumbs[2]->url}&modelId={$breadcrumbs[3]->url}&groupId={$item->group_short_name}";
            $labels[] = json_decode(json_encode([
                'url' => $url,
                'index' => $item->name,
    ...
    ?>
    <div class="list-group list-item" style='text-align: center; position: relative;'>
        <?php if ($prev) { ?>
            <a class="group-prev" href="<?=$prevUrl ?>" title="<?=$prev->name ?>">
                <span class="list-group-prev"></span>
            </a>
        <?php } ?>
        <h1 class="title" style="margin: 0 50px; display: inline-block;"><?=$title ?></h1>
        <?php if ($next) { ?>
            <a class="group-next" href="<?=$nextUrl ?>" title="<?=$next->name ?>">
            ...
            <div class="main-image-area">
                ...
                <div class="main-image imageArea" id="imageArea">
                    <span class="imageLayout" id="imageLayout">
                        <img src="<?=$hrefPrefix."?type={$model->type}&mark={$model->mark_short_name}&modelId={$model->short_name}&groupId={$group->short_name}&image=1"?>">
                                ...
                                <?php if (property_exists($coordinate,'url')) { ?>
                                    <a class="ladel a2d"
                                       ...
                                       style="position:absolute; padding:1px 5px;
                                        left: <?=$coordinate->topX ?>px;
                                        top: <?=$coordinate->topY ?>px;
                                        min-width: <?=$coordinate->bottomX - $coordinate->topX ?>px;
                                        min-height: <?=$coordinate->bottomY - $coordinate->topY ?>px;
                                        line-height: <?=$coordinate->bottomY - $coordinate->topY ?>px;
                                        font-size: <?=$coordinate->bottomY - $coordinate->topY - 2 ?>px;"
                                    ><?=$coordinate->index?></a>
                                <?php } elseif($coordinate->vertical) { ?>
                                    <span class="ladel a2d"
                                          data-left="<?=$coordinate->topX ?>"
                                          data-top="<?=$coordinate->topY ?>"
                                          title="<?=$coordinate->title ?>"
                                          data-index="<?=$coordinate->index ?>"
                                          style="position:absolute; padding:2px 1px;
                                                left: <?=$coordinate->topX ?>px;
                                                top: <?=$coordinate->topY ?>px;
                                                min-width: <?=$coordinate->bottomX - $coordinate->topX ?>px;
                                                min-height: <?=$coordinate->bottomY - $coordinate->topY ?>px;
                                                line-height:  <?=$coordinate->bottomX - $coordinate->topX ?>px;
                                                font-size:  <?=$coordinate->bottomX - $coordinate->topX - 2 ?>px;">
                                        <span style="writing-mode: vertical-rl; "><?=$coordinate->index ?></span>
                                    </span>
                                <?php } else { ?>
                                    <span class="ladel a2d"
                                          data-left="<?=$coordinate->topX ?>"
    ...
    <div class="catalog-table">
        <table class="table_main">
            <thead>
                <tr>
                    <th class="th_num">№</th>
                    <th class="th_entmy">&nbsp;</th>
                    <th class="th_num">Номер</th>
                    <th class="th_name l">Название</th>
                </tr>
            </thead>
            <tbody>
                ...
                <?php foreach ($numbers as $index => $item) {
                    ...
                    <tr class="table-item obj item-obj" data-index="<?=$item->index ?>">
                        <td class="th_num s"><?=$item->index ?></td>
                        <td class="th_entmy number-info-cell" data-number-info="<?=$index?>">
                            ...
                            <div class="modal-number-info">
                                ...
                                <div class="number-info">
                                    ...
                                    <div class="number-info-params">Автомобиль:<?=$model->name_with_mark ?></div>
                                    <?php } ?>
                                    <?php if ($item->relevance) { ?>
                                    <div class="number-info-params">Актуальность: <?=substr($item->relevance, 8, 2).'.'.substr($item->relevance, 5, 2).'.'.substr($item->relevance, 0, 4) ?></div>
                                    <?php } ?>
                                    <?php if ($item->modification) { ?>
                                    <div class="number-info-params">Модификации: <?=$item->modification ?></div>
                                    <?php } ?>
                                            ...
                                            <p>
                                                ...
                                                <div class="numbers_count">
                                                    <?php foreach ($item->count as $count) { ?>
                                                        <p><?=($count->count ? $count->count : '0') . ' - ' . $count->title?></p>
                                                    <?php } ?>
                                                </div>
                                            </p>
                                        </div>
                                    <?php } ?>
                                    <?php if ($item->notes) {?>
                                        <div class="number-info-params"><?=$item->notes ?></div>
                                    <?php } ?>
                                </div>
                            </div>
                        </td>
                        <td class="th_num s"><?=$item->number ?></td>
