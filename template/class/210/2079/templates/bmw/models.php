                            ...
                            <div class="modal-pin box-modal" id="option-modal" data-modal_option="<?php echo "{$key1}-{$key2}-{$key3}"?>">
                                ...
                                <div class="modal-option">
                                    <form class="options" id="options" action="" method="post">
                                        ...
                                        <div class="option">
                                            <input class="option-value" <?php echo $key4 === 0 ? 'checked' : '' ?> id="option<?php echo "{$key1}-{$key2}-{$key3}-{$key4}" ?>"  name="option" type="radio" value="<?=$hrefPrefix."?type={$breadcrumbs[1]->url}&mark={$breadcrumbs[2]->url}&series={$model->series_short_name}&model={$model->short_name}&rule={$option->rule_position}&transmission={$option->transmissions[0]}"?>"/>
                                            ...
                                            <?php while ($y <= $end) { ?>
                                                <div class="year-line">
                                                    ...
                                                    <span class="months">
                                                        ...
                                                        while ($m <= $mn) { ?>
                                                            <span class="month to-group" data-value="<?="{$y}"?>-<?=($m < 10 ? '0'.$m : $m)?>"><?=($m < 10 ? '0'.$m : $m)?></span>
                                                        <?php $m++;}?>
                                                    </span>
                                                </div>
                                            <?php $y++;} ?>
